...
public class Field implements Serializable {
	...
	public void move(int row, int column) {
		...
		if (emptyPosition.getRow() == row - 1 & emptyPosition.getColumn() == column) {
			Position newEmptyPosition = new Position(emptyPosition.getRow() + 1, emptyPosition.getColumn());
			setTile(emptyPosition, getTile(newEmptyPosition));
			setTile(newEmptyPosition, 0);
			emptyPosition = newEmptyPosition;
		} else if (emptyPosition.getRow() == row + 1 & emptyPosition.getColumn() == column) {
			Position newEmptyPosition = new Position(emptyPosition.getRow() - 1, emptyPosition.getColumn());
			setTile(emptyPosition, getTile(newEmptyPosition));
			setTile(newEmptyPosition, 0);
			emptyPosition = newEmptyPosition;
		} else if (emptyPosition.getRow() == row & emptyPosition.getColumn() == column + 1) {
			Position newEmptyPosition = new Position(emptyPosition.getRow(), emptyPosition.getColumn() - 1);
			setTile(emptyPosition, getTile(newEmptyPosition));
			setTile(newEmptyPosition, 0);
			emptyPosition = newEmptyPosition;
		} else if (emptyPosition.getRow() == row & emptyPosition.getColumn() == column - 1) {
	...
	private void moveWithOffset(int rowOffset, int columnOffset) {
